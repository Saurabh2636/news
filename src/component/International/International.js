...
const International = () => {
  ...
  useEffect(() => {
    const api =
      "https://gnews.io/api/v4/top-headlines?topic=health&lang=hi&county=in&token=a4e7b89d3fb9419f00fc7d26b755f0ec";
    axios
      .get(api)
      .then((response) => {
  ...
  return (
    <div>
      <Header/>
      <h1 style={{textAlign: 'center',color: 'white',fontSize:'3rem' }}>Health Today</h1>
      <div style={{ maxWidth: "180vh", margin: "auto" }}>
        <Grid container spacing={4} justifyContent="center" alignItems="center">
          {data.map((item, i) => {
            return (
              <Grid item key={i}>
                <Card className="card" style={{ maxWidth: "28rem", minHeight: "30rem"}}>
                  <Card.Img
                    variant="top"
                    src={item.image}
                    height={200}
